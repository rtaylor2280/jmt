      ...
      const [row] = await sql`
        ...
        VALUES
          (${order_number||null}, ${item_name}, ${variant||null},
           ...
           ${qty}, ${unit_cost||0}, ${shipping_allocated||0}, ${tax_allocated||0},
           ${condition||'New'}, ${category||null}, ${location||null}, ${notes||null},
           ${stockId}, ${digital})
        RETURNING *`;

      // Only touch stock if not a digital asset
      if (stockId) {
        await sql`UPDATE stock_items SET qty_on_hand = qty_on_hand + ${qty}, active = true, updated_at = NOW() WHERE id = ${stockId}`;
