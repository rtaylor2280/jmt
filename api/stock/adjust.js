...
export default async function handler(req, res) {
  ...
  try {
    ...
    // For adjustments, use current avg_cost as the cost basis
    const newAvg = calcAvgCost(item.avg_cost, item.qty_on_hand, parseInt(qty_change), Number(item.avg_cost));
    ...
    await sql`INSERT INTO stock_ledger (stock_item_id, qty_change, reason, notes)
